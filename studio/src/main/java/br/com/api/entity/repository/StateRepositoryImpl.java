...
@Repository
public class StateRepositoryImpl extends RepositoryCustom {

	@PersistenceContext
	protected EntityManager entityManager;

	public Predicate filters(StateFilter filter, CriteriaBuilder criteriaBuilder, Root<State> entity) {
		...
		if (Objects.nonNull(filter.getExample())) {
			...
			if (Objects.nonNull(filter.getExample().getCountry())) {

				countryJoin = super.setJoin(entity, "country", JoinType.LEFT);
				if (Objects.nonNull(filter.getExample().getCountry().getIdentifier())) {
					predicate = criteriaBuilder.and(predicate,
							criteriaBuilder.equal(countryJoin.get("identifier"), filter.getExample().getCountry().getIdentifier()));
				}
			}
		}
	...
	public Page<State> findByFilter(StateFilter filter, Pageable page)
			throws NoSuchMethodException, SecurityException, IllegalArgumentException, IllegalAccessException,
			InvocationTargetException, InstantiationException {

		super.findByFilter(State.class, filter);

		CriteriaBuilder criteriaBuilder = entityManager.getCriteriaBuilder();
		CriteriaQuery<Tuple> criteriaQueryTuple = criteriaBuilder.createQuery(Tuple.class);
		Root<State> root = criteriaQueryTuple.from(State.class);

		Long total = count(filter);

		Predicate conditions = this.filters(filter, criteriaBuilder, root);
		List<Selection<?>> selectionsList = super.selections(this, State.class, filter.getColumnList(),
				"state.", root, criteriaQueryTuple, criteriaBuilder);

		criteriaQueryTuple.multiselect(selectionsList);
		criteriaQueryTuple.where(conditions);

		criteriaQueryTuple.orderBy(this.orderBy(filter, root, State.class, criteriaBuilder));
		...
		List<State> rootList = new ArrayList<>();
		for (Tuple tuple : typedQuery.getResultList()) {
			rootList.add(
					(State) super.generateEntity(State.class, filter.getColumnList(), "state.", tuple));
		}
		return new PageImpl<>(rootList, page, total);
	}

	public List<State> findByFilter(StateFilter filter) throws NoSuchMethodException, SecurityException,
