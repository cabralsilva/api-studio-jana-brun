...
import br.com.api.flow.billtopay.item.BuildInstallmentsToPayFlowItem;
import br.com.api.flow.billtopay.item.FindBillToPayByFilterFlowItem;
import br.com.api.flow.billtopay.item.SetPaymentConditionToPayFlowItem;
...
@Service
public class SetPaymentConditionToBillToPayFlow {

	@Autowired
	private FindBillToPayByFilterFlowItem findBillToPayByFilterFlowItem;

	@Autowired
	private SetPaymentConditionToPayFlowItem setPaymentConditionFlowItem;
	@Autowired
	private CheckAndGetPaymentConditionFlowItem checkAndGetPaymentConditionFlowItem;

	@Autowired
	private BuildInstallmentsToPayFlowItem buildInstallmentsFlowItem;

	@Transactional
	public ResponseAPI execute(Integer billToPayIdentifier, Integer paymentConditionIdentifier,
			HttpHeaders headers) {

		ResponseAPI response = ResponseAPI.builder().friendlyMessagesList(new ArrayList<>()).build();

		try {
			final var existing = findBillToPayByFilterFlowItem.findByFilter(BillToPayFilter.builder()
					.columnList(Arrays.asList("identifier", "value", "paymentCondition.identifier")).resultUnique(true)
					.example(BillToPayDTO.builder().identifier(billToPayIdentifier).build()).build())
			...
			if (existing.isPresent()) {
				if (Objects.isNull(existing.get().getPaymentCondition())) {
					...
					setPaymentConditionFlowItem.update(billToPayIdentifier, paymentConditionIdentifier);
