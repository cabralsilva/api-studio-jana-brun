...
import br.com.api.flow.billtopayinstallment.item.FindBillToPayInstallmentByFilterFlowItem;
import br.com.api.utils.ReportTech;
import br.com.api.utils.ResponseAPI;
import br.com.api.utils.Utils;

@Service
public class FindBillToPayInstallmentByFilterFlow {

	@Autowired
	private FindBillToPayInstallmentByFilterFlowItem findBillToPayInstallmentByFilterFlowItem;

	@Autowired
	private MessageSource messageSource;

	public ResponseAPI execute(BillToPayInstallmentFilter filter, HttpHeaders headers) {

		ResponseAPI response = ResponseAPI.builder().friendlyMessagesList(new ArrayList<>()).build();

		try {
			response.setData(findBillToPayInstallmentByFilterFlowItem.findByFilter(filter));
