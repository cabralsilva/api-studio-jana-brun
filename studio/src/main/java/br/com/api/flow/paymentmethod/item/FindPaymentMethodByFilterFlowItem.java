...
import br.com.api.entity.repository.PaymentMethodRepositoryImpl;
import br.com.api.exceptions.FindByFilterException;

@Component
public class FindPaymentMethodByFilterFlowItem {
	...
	public PaymentMethodFilter findByFilter(PaymentMethodFilter filter)
		...
		if (Boolean.TRUE.equals(filter.getPageable())) {
			Page<PaymentMethod> entities = paymentMethodRepositoryImpl.findByFilter(filter,
					PageRequest.of(filter.getCurrentPage(), filter.getSizePage()));

			filter.setResult(entities.map(entity -> paymentMethodMapper.toDTO(entity)).toList());
			...
			List<PaymentMethod> entities = paymentMethodRepositoryImpl.findByFilter(filter);
			filter.setResult(
					entities.stream().map(entity -> paymentMethodMapper.toDTO(entity)).collect(Collectors.toList()));
		}
