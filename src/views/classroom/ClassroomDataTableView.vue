  ...
  <div>
    ...
    >
      <template v-slot:[`item.actions`]="{ item }">
        <v-icon small class="ma-2" @click="navigation('/panel/register/classroom/edit/' + item.identifier)">mdi-pencil</v-icon>
...
export default Vue.extend({
  ...
  methods: {
    ...
    getDataFromApi () {
      const search = {
        columnList: ['identifier', 'description'],
      ...
      this.loading = true
      httpAPI.post('/classroom/search', search)
        .then((response) => {
