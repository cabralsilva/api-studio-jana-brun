  ...
  <div>
    ...
    >
      <template v-slot:[`item.actions`]="{ item }">
        <v-icon small class="ma-2" @click="navigation('/panel/register/class/edit/' + item.identifier)">mdi-pencil</v-icon>
...
export default Vue.extend({
  ...
  methods: {
    ...
    getDataFromApi () {
      const search = {
        example: {},
        columnList: ['identifier', 'code', 'description', 'startDate', 'endDate'],
      ...
      this.loading = true
      httpAPI.post('/class/search', search)
        .then((response) => {
