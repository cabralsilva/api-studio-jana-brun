  ...
  <div>
    ...
    >
      <template v-slot:[`item.actions`]="{ item }">
        <v-icon small class="ma-2" @click="navigation('/panel/register/notice/edit/' + item.identifier)">mdi-pencil</v-icon>
...
import { TypeOfNoticeEnum, TypeOfNoticeType } from '@/model/TypeOfNoticeEnum'
import { StatusActiveEnum, StatusActiveType } from '@/model/StatusActiveEnum'
import moment from 'moment'

export default Vue.extend({
  name: 'AppEmployeeDataTableView',
  components: {},
  props: {},
  data () {
    return {
  ...
  methods: {
    ...
    getDataFromApi () {
      const search = {
        columnList: ['identifier', 'title', 'description', 'type', 'level', 'status', 'startDate', 'endDate'],
      ...
      this.loading = true
      httpAPI.post('/notice/search', search)
        .then((response) => {
          this.items = Object.keys(response.data.data.result).map(index => {
            const item = {
              ...
              level: response.data.data.result[index].level,
              type: TypeOfNoticeEnum[response.data.data.result[index].type as TypeOfNoticeType],
              status: StatusActiveEnum[response.data.data.result[index].status as StatusActiveType],
              startDate: moment(response.data.data.result[index].startDate).format('DD/MM/YYYY'),
              endDate: moment(response.data.data.result[index].endDate).format('DD/MM/YYYY')
            }
