  ...
  <div>
    ...
    >
      <template v-slot:[`item.actions`]="{ item }">
        <v-icon small class="ma-2" @click="navigation('/panel/financial/bill-to-pay/edit/' + item.identifier)">mdi-pencil</v-icon>
...
export default Vue.extend({
  ...
  data () {
    return {
      ...
      headers: [
        ...
        { text: 'Pessoa/Fornecedor', value: 'person.name' },
  ...
  methods: {
    ...
    getDataFromApi () {
      ...
      this.loading = true
      httpAPI.post('/bill-to-pay/search', search)
        .then((response) => {
          this.items = Object.keys(response.data.data.result).map(index => {
            const item = {
              ...
              value: response.data.data.result[index].value,
