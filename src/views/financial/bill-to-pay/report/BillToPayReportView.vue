        ...
        <v-card class="pa-2">
          <v-form ref="formFilter" lazy-validation>
            <v-row>
              ...
              <v-col>
                <v-select label="Pessoa/Fornecedor" v-model="formSearchFilter.personInList" :items="personInList" item-text="name" item-value="identifier" return-object multiple/>
              </v-col>
              <v-col>
...
export default Vue.extend({
  ...
  data () {
    return {
      ...
      reportHeaders: [
        ...
        { text: 'Pessoa/Fornecedor', value: 'billToPay.person.name' },
  ...
  methods: {
    ...
    async addPayment (payment: any) {
      console.log(payment)
      const response = await httpAPI.post('/bill-to-pay-installment-payment', payment)
      this.search()
    },
    ...
    async search () {
      ...
      const search = {
        ...
        asc: true,
        columnList: ['identifier', 'description', 'number', 'billToPay.person.name', 'targetDate', 'status', 'value'],
        pageable: false
      }
      const response = await httpAPI.post('/bill-to-pay-installment/search', search)
