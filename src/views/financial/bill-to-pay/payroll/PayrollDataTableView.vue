  ...
  <div>
    ...
    >
      <template v-slot:[`item.actions`]="{ item }">
        <v-icon small class="ma-2" @click="navigation('/panel/financial/bill-to-pay/payroll/edit/' + item.identifier)">mdi-pencil</v-icon>
...
import { StatusOfPayrollEnum, StatusOfPayrollType } from '@/model/StatusOfPayrollEnum'
...
export default Vue.extend({
  ...
  data () {
    return {
      ...
      headers: [
        ...
        { text: 'De', value: 'initDate' },
        { text: 'Até', value: 'endDate' },
  ...
  methods: {
    ...
    getDataFromApi () {
      ...
      this.loading = true
      httpAPI.post('/payroll/search', search)
        .then((response) => {
          this.items = Object.keys(response.data.data.result).map(index => {
            const item = {
              ...
              initDate: moment(response.data.data.result[index].initDate).format('DD/MM/YYYY'),
              endDate: moment(response.data.data.result[index].endDate).format('DD/MM/YYYY'),
              status: StatusOfPayrollEnum[response.data.data.result[index].status as StatusOfPayrollType]
            }
