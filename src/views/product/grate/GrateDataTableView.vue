  ...
  <div>
    ...
    >
      <template v-slot:[`item.actions`]="{ item }">
        <v-icon small class="ma-2" @click="navigation('/panel/register/product/grate/edit/' + item.identifier)">mdi-pencil</v-icon>
...
export default Vue.extend({
  ...
  data () {
    return {
      ...
      headers: [
        ...
        { text: 'Tipo de valor', value: 'typeOfValue' },
  ...
  methods: {
    ...
    getDataFromApi () {
      const search = {
        columnList: ['identifier', 'code', 'description', 'typeOfValue', 'status'],
      ...
      this.loading = true
      httpAPI.post('/grate/search', search)
        .then((response) => {
          this.items = Object.keys(response.data.data.result).map(index => {
            const item = {
              ...
              typeOfValue: TypeOfValueEnum[response.data.data.result[index].typeOfValue as TypeOfValueType],
