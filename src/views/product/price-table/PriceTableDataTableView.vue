  ...
  <div>
    ...
    >
      <template v-slot:[`item.actions`]="{ item }">
        <v-icon small class="ma-2" @click="navigation('/panel/register/product/price-table/edit/' + item.identifier)">mdi-pencil</v-icon>
...
import { TypeOfValueEnum, TypeOfValueType } from '@/model/TypeOfValueEnum'
import moment from 'moment'

export default Vue.extend({
  name: 'PriceTableTableView',
  components: {},
  props: {},
  data () {
    return {
      ...
      headers: [
        ...
        { text: 'Data início', value: 'initDateTime' },
        { text: 'Data término', value: 'endDateTime' },
  ...
  methods: {
    ...
    getDataFromApi () {
      ...
      this.loading = true
      httpAPI.post('/price-table/search', search)
        .then((response) => {
          this.items = Object.keys(response.data.data.result).map(index => {
            const item = {
              ...
              initDateTime: moment(response.data.data.result[index].initDateTime).format('DD/MM/YYYY HH:mm'),
              endDateTime: moment(response.data.data.result[index].endDateTime).format('DD/MM/YYYY HH:mm')
            }
