  ...
  <div>
    ...
    >
      <template v-slot:[`item.actions`]="{ item }">
        <v-icon small class="ma-2" @click="navigation('/panel/register/person/employee/edit/' + item.identifier)">mdi-pencil</v-icon>
...
import { EmployeeStatusEnum, EmployeeStatusType } from '@/model/EmployeeStatusEnum'
import { JobEnum, JobType } from '@/model/JobEnum'
...
export default Vue.extend({
  ...
  data () {
    return {
      ...
      headers: [
        ...
        { text: 'Cargo', value: 'job' },
        { text: 'Admissão', value: 'initDate' },
  ...
  methods: {
    ...
    getDataFromApi () {
      ...
      this.loading = true
      httpAPI.post('/employee/search', search)
        .then((response) => {
          this.items = Object.keys(response.data.data.result).map(index => {
            const item = {
              ...
              job: JobEnum[response.data.data.result[index].job as JobType],
              admissionDate: response.data.data.result[index].admissionDate,
              status: EmployeeStatusEnum[response.data.data.result[index].status as EmployeeStatusType]
            }
