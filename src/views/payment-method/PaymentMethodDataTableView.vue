  ...
  <div>
    ...
    >
      <template v-slot:[`item.actions`]="{ item }">
        <v-icon small class="ma-2" @click="navigation('/panel/register/financial/payment-method/edit/' + item.identifier)">mdi-pencil</v-icon>
...
export default Vue.extend({
  ...
  data () {
    return {
      ...
      headers: [
        ...
        { text: 'Tipo de moeda', value: 'currencyType' },
  ...
  methods: {
    ...
    getDataFromApi () {
      const search = {
        columnList: ['identifier', 'code', 'description', 'currencyType', 'status'],
      ...
      this.loading = true
      httpAPI.post('/payment-method/search', search)
        .then((response) => {
          this.items = Object.keys(response.data.data.result).map(index => {
            const item = {
              ...
              currencyType: TypeOfCurrencyEnum[response.data.data.result[index].currencyType as TypeOfCurrencyType],
